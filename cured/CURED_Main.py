...
def validate_args(ARGS):
    if bool(vars(ARGS)["sequence_type"]) and not bool(vars(ARGS)["species"]):
        SystemExit.exit("Error: You have to use --sequence_type with --species\n")
    if bool(vars(ARGS)['database']) and not bool(vars(ARGS)['species']):
        SystemExit.exit('Error: When using --database option, must use --species\n')
    if bool(vars(ARGS)['case_accession_list']) and not bool(vars(ARGS)['species']):
        SystemExit.exit('Error: When using --case_accession_list option, must use --species\n')
    if bool(vars(ARGS)['species']) and bool(vars(ARGS)['genomes_folder']) and not bool(vars(ARGS)['case_genomes']):
        SystemExit.exit('Error: When using --case-genomes option, must use --species and --genomes_folder\n')
    if bool(vars(ARGS)['use_datasets']) and not bool(vars(ARGS)['case_control_file']):
        SystemExit.exit('Error: When using --use_datasets option, must provide a case/control file using --case_control_file \n')
    if bool(vars(ARGS)['use_simple']) and not bool(vars(ARGS)['kmer_list']):
        SystemExit.exit('Error: When using --use_simple option, must provide a file of k-mers using --kmer_list\n')
...
def check_dependency():
    try:
        ...
        SystemExit.exit('ERROR: unitig-caller cannot be found. Please install.')
    try:
        ...
        SystemExit.exit('ERROR: mlst cannot be found. Please install.')
    try:
        ...
        SystemExit.exit('ERROR: ncbi-datasets cannot be found. Please install.')

def setup_environment(args):
    # assign defaults that are used throughout
    case_control = args.case_control_file
    thr = args.threads or 1
    ...
    LOG_FILE = f"CURED_{TIMESTR}"
    handlers = [logging.FileHandler(f"{LOG_FILE}.log")]
    if not args.quiet:
    ...
    return {
        ...
        "EXT": EXT,
        "db": db,
        "extractedFiles_directory": extractedFiles_directory,
...
def download_genomes_taxon(genus_species_command,db,extractedFiles_directory):
    max_retries = 15
...
def run_mlst(genome_dict, EXT, sequence_type,extractedFiles_directory):
...
def write_case_control_file(TIMESTR, genome_dict, EXT):
...
def define_sensitivity_and_specificity(sensitivity, specificity, num_cases, num_controls):
...
def generate_unitig_caller_cmd(unitig_counter, thr, fop, master_files_lst, kmer_length, ARGS, kmer_list):
    if unitig_counter == 0:
        if ARGS.use_simple:
...
def check_num_lines(fop,unitig_counter):
...
def main(ARGS):
    ...
    # Variables from environment set up 
    env = setup_environment(ARGS)
    case_control = env["case_control"]
    thr = env["thr"]
    kmer_length = env["kmer_length"]
    EXT = env["EXT"]
    db = env["db"]
    extractedFiles_directory = env["extractedFiles_directory"]
    genus_species = env["genus_species"]
    ...
    if ARGS.species:
        ...
        if ARGS.case_accession_list:
            ...
            download_genomes_taxon(genus_species_command, db, extractedFiles_directory)
            ...
            if value_to_check in genome_dict.values():
                ...
                download_genomes_accession(accessions_to_download, extractedFiles_directory)
            ...
            if ARGS.sequence_type:
                download_genomes_taxon(genus_species_command, db, extractedFiles_directory)
                run_mlst(genome_dict, EXT, sequence_type, extractedFiles_directory)
                #os.remove('mlst_report.tsv')

        write_case_control_file(TIMESTR, genome_dict, EXT)
    ################################################################################
    ...
    global_sensitivity_threshold, global_specificity_threshold, specificity_step2, sensitivity_step1 = define_sensitivity_and_specificity(sensitivity, specificity, num_cases, num_controls)
    ...
    for sublist in genome_lists:
        ...
        logging.info("Wrote refs.txt")

        # Run unitig-caller. Unzip the output.
        TIMESTR = time.strftime("%Y%m%d_%H%M%S")
        fop = "unitig_caller_{}_{}".format(str(unitig_counter),TIMESTR)
        fop = os.path.join(temp_dir, fop)
        unitig_caller_cmd = generate_unitig_caller_cmd(unitig_counter, thr, fop, master_files_lst, kmer_length, ARGS, kmer_list)
        
        logging.info("Wrote unitig caller command")
