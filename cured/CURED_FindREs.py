...
def add_arguments(PARSER):
    ...
    type=str, required=True, help='Csv file of cases and controls.')
...
def setup_environment(ARGS):
    ...
    "MM2_PARAMS_SAFE": MM2_PARAMS_SAFE,
    "MM2_PARAMS_CASE": MM2_PARAMS_CASE
    }
...
def check_dependencies():
    ...
    try:
        minimap2_check = subprocess.run(['minimap2', '--version'], capture_output=True, text=True)
        logging.info('Found minimap2.')
    except FileNotFoundError:
        logging.error('Minimap2 cannot be found. Please install.\n')
        SystemExit.exit('Minimap2 cannot be found. Please install.\n')
...
def write_PCR_report(pcr_dict, unique_kmer, filepath, unique_case_regions, output_dir):
    ...
    temp_pcr_path = os.path.join(output_dir, f'{khash}_pcr.fa')
...
def main(ARGS):
    ...
    SAFE_FLAGS = params['SAFE_FLAGS']
    MM2_PARAMS_SAFE = params['MM2_PARAMS_SAFE']
    MM2_PARAMS_CASE = params['MM2_PARAMS_CASE']
    ...
    for kmer, data in query_data.items():
        ...
        if final_enzymes:
            ...
            write_PCR_report(pcr_dict, kmer, filepath, unique_case_regions, ARGS.output_dir)
