...
def check_dependencies():
    try:
        ...
        SystemExit.exit('blast cannot be found. Please install.\n')
    try:
        ...
        SystemExit.exit('samtools cannot be found. Please install.\n')
    try:
        ...
        SystemExit.exit('bwa cannot be found. Please install.\n')
    try:
        ...
        SystemExit.exit('Biopython cannot be found. Please install.\n')
...
def parse_bwa_output(bwa_case_output_lines, filepath, unique_kmer, added_bases, case_genome_to_check, pcr_dict, include_upstream, include_downstream):
    try:
...
def parse_blastn_short(blast_output_WS, added_bases, pcr_dict, filepath, unique_kmer, include_upstream, include_downstream):
    ...
    if len(blast_output_WS) < 5:
        logging.warning('No alignment found with blastn-short.')
        return None
...
def main(ARGS):
    ...
    added_bases = params['added_bases']
    accession_numbers = params['accession_numbers']
    include_upstream = params['include_upstream']
    include_downstream = params['include_downstream']
    ...
    for unique_kmer in unique_kmers:
        ...
        if dict_of_found_enzymes: # If REs were found in the unique kmer, execute this block of code.
            ...
            bwa_output_case_list = parse_bwa_output(bwa_output_case, filepath, unique_kmer, added_bases, case_genome_to_check, pcr_dict, include_upstream, include_downstream)

            if bwa_output_case_list == None:
                blastn_output = run_blastn_short(unique_kmer, filepath)
                blastn_output_parsed = parse_blastn_short(blastn_output, added_bases, pcr_dict, filepath, unique_kmer, include_upstream, include_downstream)
                
                if  blastn_output_parsed == None:
                    logging.warning('No alignment found in case genome with either bwa or blastn-short. Moving to next kmer...')
                    logging.info('-----------------------------------------------')
                    continue
                else: 
                    case_sequence, aln_length = blastn_output_parsed[0], blastn_output_parsed[1]
            ...
            for control in control_filepaths:
                ...
                bwa_output_control_list = parse_bwa_output(bwa_output_control, control, unique_kmer, added_bases, case_genome_to_check, pcr_dict, include_upstream, include_downstream)
